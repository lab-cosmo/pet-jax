...
Step 2 involves the two cutoff widths of the model, which are easy to confuse. To keep the procedure differentiable, neighbors are counted *softly*, with the usual smooth bump function: `n_i(r) = sum_j bump(d_ij; r, cutoff_width_adaptive) + N_target * (r / cutoff)^3`. The per-atom cutoff is the radius where this count crosses the target; the cubic baseline makes sure there is exactly one crossing. So `cutoff_width_adaptive` is the blur radius of the neighbor counting, and nothing else — the separate `cutoff_width` is used later, to taper edge features to zero at the selected pair cutoffs. The blur is not cosmetic: a blurrier count crosses the target further out and selects more neighbors, so the model has to be run with the width it was trained with. The crossing itself is found either on a probe grid with Gaussian weighting (`adaptive_cutoff_method: grid`) or by Newton-bisection root finding (`solver`), again matching the checkpoint.
...
- `matmul_precision` (default `"high"`): JAX fp32 matmul accumulation precision on accelerators (`"default"` / `"high"` / `"highest"`); a no-op on CPU. The `"default"` mode uses reduced-precision (TF32-style) accumulation that noticeably degrades NVE energy conservation; `"high"` restores it at essentially no extra cost, while `"highest"` (full fp32) is slower when compute-bound. Set to `None` to leave JAX's global config untouched (for callers managing it themselves). Like `default_dtype`'s fp64 switch, this mutates global JAX state.
...
- `direct_forces` / `direct_stress` (default `False`, passed via `from_checkpoint`): read forces/stress straight from the checkpoint's non-conservative heads instead of differentiating the energy — skips the autodiff pass entirely (no double-backward), so cheaper, especially together. Trades accuracy and energy-conservation for speed (the net force is removed to avoid drift); see [Conventions](#conventions).
