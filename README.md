...
One additional entry point serves external training pipelines (e.g. `iris`): `pack_edges` is the selection-free sibling of `truncate_edges` — fixed-width packing of an NL already trimmed upstream (hard-capped neighbor counts; the model then applies its plain scalar-cutoff bump via `pair_cutoffs=None`). Pipelines that need more than `UPET`'s outputs (e.g. the node embedding for composite models) recompose the exported pieces — `Backbone`, `Energy`, `DirectForces`, `DirectStress` — rather than switching `UPET`'s behavior; that is what the decomposition is for.
