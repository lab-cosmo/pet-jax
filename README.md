...
- `num_neighbors_adaptive` (default `None`): override the per-atom neighbour target the adaptive cutoff selects for; `None` uses the trained `config["num_neighbors_adaptive"]`. Raising it widens the per-atom adaptive cutoffs (still bounded by the trained `config["cutoff"]`, so the raw NL is unaffected) and selects more neighbours per atom — models are robust to this and accuracy improves, at the cost of a larger `k_sel` and modestly higher per-step compute. The value feeds both the `k_sel` sizing and the in-JIT forward, so they always agree. (If combined with a tight `cutoff_override`, the wider reach is more likely to trip the `cutoff_override` warning below.)
